...
// ВНИМАНИЕ: Я добавил Trophy и все остальные иконки в этот список. Теперь ошибки не будет.
import { 
  Trophy, 
  Users, 
  Plus, 
...
export const Lobby = ({ 
  ...
  // Получаем комнаты из твоего Redis
  ...
  return (
    <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-white/10 shadow-2xl min-w-[350px]">
      <div className="flex flex-col items-center mb-10">
        ...
        <h1 className="text-3xl font-black text-white italic uppercase tracking-tighter text-center">Maze Race</h1>
      </div>
      ...
      {view === 'MENU' ? (
        <div className="space-y-4">
          <input 
            ...
            className="w-full bg-slate-800 border-2 border-slate-700 p-4 rounded-2xl text-white font-bold text-center outline-none focus:border-blue-500" 
          />

          <button 
            onClick={onHost} 
            disabled={!isPeerReady || !playerName.trim()} 
            className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-black p-5 rounded-2xl flex items-center justify-center gap-3 active:scale-95 shadow-lg shadow-blue-600/25"
          >
          ...
          <button 
            ...
            className="w-full bg-slate-800 hover:bg-slate-700 text-white font-black p-5 rounded-2xl flex items-center justify-center gap-3 active:scale-95"
          >
            ...
            {rooms.length > 0 ? (
              rooms.map((room) => (
                <div key={room.id} className="bg-slate-800/40 border border-slate-700 p-4 rounded-2xl flex items-center justify-between group hover:border-blue-500 transition-all">
                  <div>
                    <p className="text-white font-bold">{room.hostName || 'Игрок'}</p>
              ...
              <div className="text-center py-10">
                <p className="text-slate-600 italic text-sm font-medium">Комнат пока нет...</p>
              </div>
            )}
          </div>
        </div>
